...
preTrainedModelName = "/root/da660/model/clip-ViT-L-14"
model = SentenceTransformer(preTrainedModelName)

@app.route('/searchSimilar', methods=['GET'])
def searchSimilar():
    vector_img_path = request.args.get('vector',  default=None,type=str)
    if vector_img_path.startswith("http"):
        img = Image.open(requests.get(vector_img_path, stream=True).raw)
    else:
        img = Image.open(vector_img_path)
    
    vector_img_query = model.encode(img).tolist()
